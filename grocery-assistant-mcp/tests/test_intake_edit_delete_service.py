...
def intake_edit_delete_csvs(tmp_path, monkeypatch):
    """
    Create isolated temporary CSV files for Version 1.2 intake edit/delete tests.

    This prevents tests from touching the real project data files.
    """
...
def test_intake_entry_has_items_returns_true_when_children_exist(
...
def test_update_intake_entry_rejects_unknown_intake_id(intake_edit_delete_csvs):
    with pytest.raises(ValueError, match="No intake entry found"):
        service.update_intake_entry(
            intake_id="intake_999",
            meal_name="Should fail",
        )
...
def test_update_intake_entry_rejects_negative_nutrition_value(
    intake_edit_delete_csvs,
):
    with pytest.raises(ValueError, match="total_calories_estimate"):
        service.update_intake_entry(
            intake_id="intake_001",
            total_calories_estimate=-1,
        )
...
def test_update_intake_item_updates_only_supplied_fields(intake_edit_delete_csvs):
    ...
    # Unchanged relationship fields should be preserved.
    assert row["intake_id"] == "intake_001"
    assert row["stock_id"] == "inv_001"


def test_update_intake_item_can_move_item_to_existing_parent(
    intake_edit_delete_csvs,
):
    result = service.update_intake_item(
        intake_item_id="intake_item_001",
...
def test_update_intake_item_rejects_unknown_intake_item_id(
    intake_edit_delete_csvs,
):
    with pytest.raises(ValueError, match="No intake item found"):
        service.update_intake_item(
            intake_item_id="intake_item_999",
            food_item="Should fail",
        )


def test_update_intake_item_rejects_unknown_parent_intake_id(
    intake_edit_delete_csvs,
):
    with pytest.raises(ValueError, match="No intake entry found"):
        service.update_intake_item(
...
def test_update_intake_item_rejects_negative_numeric_value(
    intake_edit_delete_csvs,
):
...
def test_remove_intake_item_removes_child_but_keeps_parent(
    intake_edit_delete_csvs,
):
...
def test_remove_intake_entry_blocks_when_child_items_exist(
    intake_edit_delete_csvs,
):
...
def test_remove_intake_entry_after_removing_all_child_items(
    intake_edit_delete_csvs,
):
