...
def test_inventory_consumption_records_can_be_converted_to_json():
    df = read_inventory_consumption()
    records = df_to_records(df)
    payload = to_json(records)
    parsed = json.loads(payload)
    assert isinstance(payload, str)
...
def test_list_inventory_consumption_payload_is_valid_json():
    items = list_inventory_consumption()
    payload = to_json(items)
    parsed = json.loads(payload)
    assert isinstance(parsed, list)
